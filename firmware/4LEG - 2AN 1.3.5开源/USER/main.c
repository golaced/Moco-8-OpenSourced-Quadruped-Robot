...
{ 
    ...
    Beep_Init(0,84-1);
//------------------------Uart Init-------------------------------------
		//Usart1_Init(115200);			//外部控制  
		//Usart1_Init(2000000);			//ANO_A 数传
		//Usart1_Init(38400L);			//3DR 数传
		Usart1_Init(57600);				  //WIFI 数传
		mavlink_init();
		#if EN_DMA_UART1 
	  ...
	  Drv_GpsPin_Init();
		...
		#endif			
		Usart4_Init(115200);			//Opticalflow或外扩模组  
		#if EN_DMA_UART4 
		...
		#endif
		Usart3_Init(115200);      //RaspberryPi或图像
		#if EN_DMA_UART3
...
//	//-----------------DMA Init--------------------------
  ...
  I2c_Soft_Init_VL53();Delay_ms(100);
	Outer_Compass_init();
