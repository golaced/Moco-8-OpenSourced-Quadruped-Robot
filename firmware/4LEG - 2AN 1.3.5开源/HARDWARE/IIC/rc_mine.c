...
{	vs16 _temp;	u8 _cnt=0;u8 i;u8 sum = 0;
  ...
  _temp =  vmc_all.att_ctrl[YAWr]*10;
	...
	_temp =  (nav.pos_n[Xr]+2.5)*100;	
	NRF24L01_TXDATA[_cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[_cnt++]=BYTE0(_temp);
	_temp =  (nav.pos_n[Yr]+2.5)*100;	
	NRF24L01_TXDATA[_cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[_cnt++]=BYTE0(_temp);
	}
...
void param_copy_pid(void){
	...
	vmc_all.cog_off[F] = -0.0001*PARAM_NRF[0][0];
	vmc_all.cog_off[B] = -0.0001*PARAM_NRF[0][1];
	vmc_all.kp_deng[FL1]  =      PARAM_NRF[0][2];
...
void pid_copy_param(void){
	...
	PARAM_NRF[0][0]=ABS(vmc_all.cog_off[F])*10000;
	PARAM_NRF[0][1]=ABS(vmc_all.cog_off[B])*10000;
	PARAM_NRF[0][2]=vmc_all.kp_deng[FL1];
