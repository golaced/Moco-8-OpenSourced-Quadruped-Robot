...
#define VIR_MODEL 0  			//使用虚拟机器人模型测试导航算法  调试用
#define DEBUG_MODE 0 			//测试模式，动作放慢3倍						调试用
...
#define GLOBAL_CONTROL 0  //			全局姿态控制  0->目前比较稳定
#define DOUBLE_LOOP 1			//			串级姿态控制
#define USE_MODEL_FOR_ATT_CAL 1//	使用机械模型融合IMU计算姿态角
#define USE_ESO_OBSEVER   1//			使用ESO作为角速度观测器
#define USE_ATT_POLAR     1//			使用极坐标系角度补偿
#define USE_NEW_INV_MODLE 1//			使用包含高度的倒立摆跨脚位置模型
#define EN_PLAN_USE_JERK  2//     使用最小JERK足端轨迹规划  2->使用真实速度规划
//-------机型选择--------
#define MINI_LITTRO_DOG    //Mocomoco  机架
//#define BIG_LITTRO_DOG	 //Big moco  机架

#define MINI_TO_BIG 0    //直接使用小机器人参数放大至大机器人
//-------系统参数--------
...
typedef struct 
{
  char id,trig_state,ground_state,invert_knee;
	...
	float PWM_OUT[4];				//PWM输出
	float PWM_PER_DEGREE[4];//PWM角度增益
	int sita_flag[4];				//PWM增加方向
	float sita[4];
	float st_time_used;
	float sw_time_used;
...
typedef struct 
{
  ...
  //-----------系统变量--------------
	...
	float max_l,max_to_end;//最大腿长
	float dt_size[2];//控制周期变化导致的参数缩放比例
	char jump_state;
...
typedef struct 
{
	...
	char your_key[3];//你的授权码
	int board_id[3]; //控制器ID
	float sita_test[5];//[4]->1  强制角度测试
	//---------------------------------------
	u8 ground[2][4];
