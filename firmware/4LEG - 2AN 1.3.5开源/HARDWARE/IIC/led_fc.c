...

{
	...
	switch(idle_state)
	{
		...
		if(Rc_Get_SBUS.update||module.nrf==2) 
			if(module.hml_imu_o)
				{LEDRGB_COLOR(WHITE);fc_state_beep[0]=BEEP_STATE1;} 	
			else
		...
		case 3:
			if(pi.connect) 
			{LEDRGB_COLOR(BLUE);fc_state_beep[1]=BEEP_STATE1;} 	
			else
		...
		case 5:
			if(Gps_information.connect)
				if(Gps_information.satellite_num>5)
					{LEDRGB_COLOR(WHITE);fc_state_beep[2]=BEEP_STATE1; }
				else
					{LEDRGB_COLOR(BLUE);fc_state_beep[2]=BEEP_STATE1; }
			else
