...
/** @defgroup STM32_EVAL_SDIO_SD_Private_Defines
  * @{
  */ 
#define SD_DETECT_PIN                    GPIO_Pin_0                 /* PF.11 */
...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  //((DMA_Channel_TypeDef *)DMA1_Channel4)->CNDTR=0;
...
{
  uint32_t Status = (uint32_t)DMA_GetFlagStatus(DMA2_FLAG_TC4);
...
SD_Error SD_Init(void)
{
  ...
  if (errorstatus != SD_OK)
  {
	...
	printf("-- SD card power on error : %d\n\r",errorstatus);
    return(errorstatus);
  }
  ...
  {
    /*!< CMD Response TimeOut (wait for CMDSENT flag) */
	printf("-- SD card Initialize error : %d\n\r",errorstatus);
    return(errorstatus);
  }
  ...
  {
  	printf("-- SD_SelectDeselect error : %d\n\r",errorstatus);
  }
//  if(errorstatus == SD_OK)
//  {
...
//  else
//  {
//  	printf("-- SD_EnableWideBusOperation error : %d\n\r",errorstatus);
//  }
//  if(errorstatus != SD_OK)
//  {
//  	printf("-- SD_SetDeviceMode error : %d\n\r",errorstatus);	
//  }
  if(errorstatus != SD_OK)
  {
  	printf("-- SD_EnableWideBusOperation error : %d\n\r",errorstatus);	
  ...
  while (!(SDIO->STA & (SDIO_FLAG_DBCKEND | SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)))
  {
    if (SDIO_GetFlagStatus(SDIO_FLAG_TXFIFOHE) != RESET)
    {
      if ((512 - bytestransferred) < 32)
      {
        ...
        {
          SDIO->FIFO = *tempbuff;//SDIO_WriteData(*tempbuff);
        }
      }
      else
      {
        for (count = 0; count < 8; count++)
        {
          SDIO->FIFO = *(tempbuff + count);//SDIO_WriteData(*(tempbuff + count));
        }
        tempbuff += 8;
        bytestransferred += 32;
      }
...
{
 if ((SDIO->STA & SDIO_FLAG_DTIMEOUT) != (uint32_t)RESET)
 {			  
  //SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT);
  SDIO->ICR = SDIO_FLAG_DTIMEOUT;
  TransferError = SD_DATA_TIMEOUT;
 }
 else if ((SDIO->STA & SDIO_FLAG_DCRCFAIL) != RESET)
 {
  SDIO->ICR = SDIO_FLAG_DCRCFAIL;//SDIO_ClearFlag(SDIO_FLAG_DCRCFAIL);
  TransferError = SD_DATA_CRC_FAIL;
 }
 else if ((SDIO->STA & SDIO_FLAG_RXOVERR) != RESET)
 {
  SDIO->ICR = SDIO_FLAG_RXOVERR;//SDIO_ClearFlag(SDIO_FLAG_RXOVERR);
  TransferError = SD_RX_OVERRUN;
 }
 else if ((SDIO->STA & SDIO_FLAG_STBITERR) != RESET)
 {
  SDIO->ICR = SDIO_FLAG_STBITERR;//SDIO_ClearFlag(SDIO_FLAG_STBITERR);
